...
def execute(filters=None):
	...
	supplier_vat = get_supplier_vat(invoice_list)
	...
	for inv in invoice_list:
		...
		row = [inv.name, inv.posting_date, inv.supplier, inv.supplier_name,
			supplier_details.get(inv.supplier), supplier_vat.get(inv.supplier),
...
def get_columns(invoice_list):
	...
	columns = [
		...
		_("Supplier Type") + ":Link/Supplier Type:120", _("Tin No") + "::120",_("Payable Account") + ":Link/Account:120", 
...
def get_supplier_vat(invoice_list):
	supplier_vat = {}
	suppliers = list(set([inv.supplier for inv in invoice_list]))
	for supp in frappe.db.sql("""select name, vat_tin from `tabSupplier`
		where name in (%s)""" % ", ".join(["%s"]*len(suppliers)), tuple(suppliers), as_dict=1):
			supplier_vat.setdefault(supp.name, supp.vat_tin)
	return supplier_vat