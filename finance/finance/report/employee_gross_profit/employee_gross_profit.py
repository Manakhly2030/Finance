...
def execute(filters=None):
	...
	columns_list = ["parent", "customer", "employee_name", "posting_date", "item_code", "qty", "base_rate", "buying_rate", 
		"base_amount", "buying_amount", "gross_profit", "gross_profit_percent"]
...
def get_columns():
	columns = [
		...
		_("Employee Name") + ":Link/Employee:100",
		_("Posting Date") + ":Date:100",
		_("Item Code") + ":Link/Item:100",
		_("Qty") + ":Float:80",
		_("Avg. Selling Rate") + ":Currency:100",
		_("Avg. Buying Rate") + ":Currency:100",
		_("Selling Amount") + ":Currency:100",
		_("Buying Amount") + ":Currency:100",
		_("Gross Profit") + ":Currency:100",
		_("Gross Profit %") + ":Percent:100",
	]
...
class GrossProfitGenerator(object):
	...
	def process(self):
		...
		for row in self.si_list:
			...
			# add to grouped
			self.grouped.setdefault(row.get('invoice'), []).append(row)
	...
	def load_invoice_items(self):
		...
		if self.filters.get('employee_name'):
			conditions += " and employee_name = %(employee_name)s"

		self.si_list = frappe.db.sql("""
			select
				`tabSales Invoice Item`.parenttype, `tabSales Invoice Item`.parent,
				`tabSales Invoice`.posting_date, `tabSales Invoice`.posting_time,
				`tabSales Invoice`.update_stock, `tabSales Invoice`.customer, `tabSales Invoice`.employee_name,
				`tabSales Invoice Item`.item_code, `tabSales Invoice Item`.dn_detail,
			...
			.format(conditions=conditions, match_cond = get_match_cond('Sales Invoice')), self.filters, as_dict=1)
